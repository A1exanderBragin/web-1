...
if ($action === 'login') {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $pdo->prepare("SELECT id, password_hash FROM users5 WHERE login = ?");
        $stmt->execute([$login]);
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO users5 (fio, phone, email, birthdate, gender, bio, contract, login, password_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            // Сохранение языков
            $stmt = $pdo->prepare("SELECT id FROM programming_languages5 WHERE name = ?");
            $insert = $pdo->prepare("INSERT INTO user_languages5 (user_id, language_id) VALUES (?, ?)");
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->prepare("UPDATE users5 SET fio = ?, phone = ?, email = ?, birthdate = ?, gender = ?, bio = ?, contract = ? WHERE id = ?");
            ...
            // Обновление языков
            $pdo->prepare("DELETE FROM user_languages5 WHERE user_id = ?")->execute([$_SESSION['user_id']]);
            $stmt = $pdo->prepare("SELECT id FROM programming_languages5 WHERE name = ?");
            $insert = $pdo->prepare("INSERT INTO user_languages5 (user_id, language_id) VALUES (?, ?)");
            foreach ($languages as $language) {
                $stmt->execute([$language]);
